...
void resetStats() {
  stats1.min = 999.0; stats1.max = 0.0; stats1.total = 0.0; stats1.count = 0;
  stats2.min = 999.0; stats2.max = 0.0; stats2.total = 0.0; stats2.count = 0;
...
void animateGaugeValue(lv_obj_t *arc, int32_t new_value, int32_t *current_value) {
  if (*current_value == new_value) return; // No change needed
...
void updateStatsDisplay() {
  ...
  float packet_loss = totalPacketsReceived > 0 ? 
    (float)missedPackets / (totalPacketsReceived + missedPackets) * 100.0 : 0.0;
